...
*/
#define	ksVersionNumber		"Firmware v8.3.25"
/*

upravy pro Siton SMD (PWM LCD v5) 4/2024 by JK
verze 8.3.25
 - pridan damping (filtr) pro merení teploty (DEFAULT = 20 Sec.)
 - změna kalibrace proudu na offset nuly +/- 1.0A
verze 8.3.24
 - pridana podpora PWM rizeni podsviceni displeje
 - smazany inicializace pinu A6 a A7
 - Vcc natvrdo 5000mV
 - implementována autodetekce I2C adresy displeje (0x27 / 0x3F)
 - opětovná inicializace displeje každou hodinu
 - upraveno intro
...
5.1a- blokovani chodu menice pri nastavovani celkov� vyroby kWh
...
//#define I2C_Address		0x3F          // adresa I2C prevodniku PCF8574T,PCF8574P 0x27 (PCF8574AT 0x3F)
...
#define DAMP_SEC  20 //hodnota dampingu v sekundach (filtr teplotniho cidla)
...
float TeplBojl = 0; //skutecna teplota bojleru (TUV)
byte stupen[8] = {
	  0b01100
	, 0b10010
	, 0b10010
...
void setup() {
	...
	digitalWrite(2, LOW);
  //==============================================================================
  // PIN D3 pro rizeni úrovne podsviceni pomoci PWM
	...
	delay(60);
  // Autodetekce adresy displeje
  Wire.begin();
  while (I2C_addr == 0){
    Wire.beginTransmission (0x27); // scan the I2C bus
    if (Wire.endTransmission () == 0) {
      I2C_addr = 0x27;
      lcd = new LiquidCrystal_I2C(I2C_addr, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE); // use found I2C address for LCD display
    }   
    Wire.beginTransmission (0x3F); // scan the I2C bus
    if (Wire.endTransmission () == 0) {
      I2C_addr = 0x3F;
      lcd = new LiquidCrystal_I2C(I2C_addr, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE); // use found I2C address for LCD display
    }    
  ...
  wdt_reset(); //reset watchdogu
  lcd->setCursor(0, 0);
  lcd->print(" Tomas Chvatal  ");
  lcd->setCursor(0, 1);
  lcd->print(" www.fordiag.cz ");
  delay(1200);
  lcd->clear();
  wdt_reset(); //reset watchdogu
	/*
...
void mereni()
{
  ...
  vcc = 5000;
	...
	//Anormal = 9;//###DEBUG###
	Anormal = Anormal + (kalibA * 10); //kalibrace hodnoty proudu (offset)
	//proud = (int) Avolt;
...
hwSerial.println("");
	...
	//vykon = 3500;//###DEBUG###
	if (totT > 1020) // pokud je odpojen� termo�idlo nastav 0 oC
	{
		totT = 437;
	}

  if (TeplBojl == 0) {
      // Bez dampingu po startu a po dopojení čidla
      TeplBojl = vypocet(totT); //vypocet teploty
  }
  else {
      // Damping (filtr)
      float w = (float)1 / (0.6 + (DAMP_SEC / 0.1));
      TeplBojl = (TeplBojl * (1 - w)) + (vypocet(totT) * w);
  }
